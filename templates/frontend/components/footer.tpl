...
{if $requestedPage !== 'login' && $requestedPage !== 'user'}
  {if empty($isFullWidth)}
  	...
  	{if $sidebarCode}
  		<ul class="pkp_structure_sidebar list-unstyled container" role="complementary" aria-label="{translate|escape key="common.navigation.sidebar"}" data-aos="fade-up" data-aos-delay="200">
  			{$sidebarCode}
  		</ul><!-- pkp_sidebar.left -->
  	{/if}
  {/if}
